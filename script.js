// ===== STATE =====
const DISCORD_WEBHOOK = "https://discord.com/api/webhooks/1404263977067745332/Zwzgicl8ZDas11jkALBMwivTEUmES1Zq-GYF15wVIgwGEp7NPb7-ckM1ktNDYPQBG3IT";
var S = {
  step: 0,
  name: "",
...
function renderStepper(){
  var steps=[
    ...
    "Theme color","Name / Description / Links","Discord (only join dont do anything else)","Blur & Glow","Weather Effects",
...
var screens=[
  ...
  ()=>`<div class="row"><div>
        <label>Website name (like alqulol.xyz/(website-name))</label>
  ...
  ()=>{ const badge = (S.discordEnabled && S.discordId)?'<span class="badge good"><i class="fa-solid fa-circle-check"></i> Connected</span>':'<span class="badge"><i class="fa-brands fa-discord"></i> Optional</span>';
        ...
        <p class="hint">This is not owned by alqulol it is external you need to join another discord to appear your discord card but it is powered by alqulol.</p>
        <div class="discord-actions">
          <a class="btn btn-primary" href="https://discord.gg/hPQ5NzNJmW" target="_blank" rel="noopener"><i class="fa-brands fa-discord"></i> Join your Discord</a>
          ${badge}
        </div>
  ...
  ()=>`<h2>Generate ZIP</h2>
      <p>Click to download the folder <span class="mono">${S.name||'site'}</span> .</p>
      <button class="btn btn-primary" id="genZipBtn"><i class="fa-solid fa-box-archive"></i> downlaod the zip </button>`
  ...
  ()=>`<h2>Host & Finish</h2>
      <ol class="host-steps">
        <li><b>Step 1:</b> Click <b>Generate ZIP</b> on the previous step and download it.</li>
        <li><b>Step 2:</b> Upload the ZIP to <a href="https://gofile.io" target="_blank" rel="noopener">gofile.io</a>.</li>
        <li><b>Step 3:</b> Paste your <b>gofile link</b> below — the finish button will appear.</li>
        <li><b>Step 4:</b> Click Finish to send and see confirmation.</li>
      </ol>
      <div class="row">
        <div style="flex:1">
          <label>GoFile link</label>
          <input id="hostLinkInput" class="fancy-input" type="url" placeholder="https://gofile.io/d/XXXXXXXX">
          <div class="hint">Example: https://gofile.io/d/abcdEF12</div>
        </div>
      </div>
      <div style="margin-top:12px">
        <button class="btn btn-primary" id="finishHostBtn"><i class="fa-solid fa-rocket"></i> Finish and host the website</button>
      </div>
      <p class="hint" id="hostStatus" style="margin-top:10px"></p>
      <div id="bigNotice" class="big-notice" style="display:none">
        <div class="big-notice-card">
          <div class="big-notice-title">All set!</div>
          <div class="big-notice-body" id="bigNoticeMsg"></div>
          <button id="bigNoticeClose" class="btn btn-secondary">Close</button>
        </div>
      </div>`
...
function wireFinishHostUI(){
  const COOLDOWN_MS = 60*60*1000; // 1 hour
  let userIP = 'unknown';
  // Fetch IP best-effort
  try{ fetch('https://api.ipify.org?format=json',{cache:'no-store'})
         .then(r=>r.json()).then(j=>{ userIP = (j && j.ip) || 'unknown'; }); }catch(_){ userIP='unknown'; }
  try{
    const input = $("#hostLinkInput");
    const btn = $("#finishHostBtn");
    const status = $("#hostStatus");
    const big = $("#bigNotice");
    const bigMsg = $("#bigNoticeMsg");
    const bigClose = $("#bigNoticeClose");
    if(!btn || btn.dataset.bound==='1') return;
    // Cooldown check helper
    function key(){ return 'alqulol_send_' + (userIP||'unknown'); }
    function remaining(){ try{ const t=Number(localStorage.getItem(key())||0); return Math.max(0, COOLDOWN_MS - (Date.now()-t)); }catch(_){ return 0; } }
    function fmt(ms){ const m=Math.ceil(ms/60000); return m + ' min'; }
    function applyCooldownUI(){
      const left = remaining();
      if(left>0){ btn.disabled = true; btn.title='Cooldown'; if(status) status.textContent = 'Please wait ' + fmt(left) + ' before sending again.'; }
      else{ btn.disabled = false; if(status) status.textContent = ''; }
    }
    // initial check after small delay to allow IP fetch
    setTimeout(applyCooldownUI, 300);
    // re-check every 30s
    setInterval(applyCooldownUI, 30000);


    // Keep text in state
    if(input){
      input.addEventListener('input', ()=>{ S.hostLink = input.value.trim(); status && (status.textContent = ""); });
    }

    function showBigNotice(name){
      if(!big || !bigMsg) return;
      const url = "https://alqulol.xyz/" + (name || "unnamed-site");
      bigMsg.innerHTML = "✅ Sent! Your website will be hosted in <b>20 minutes</b>.<br>Please wait or join <b>alqulol</b> for the process.<br><br><b>Your URL will be:</b><br><code>"+url+"</code>";
      big.style.display = "flex";
    }
    if(bigClose){ bigClose.addEventListener('click', ()=>{ big.style.display = "none"; }); }

    btn.addEventListener('click', async ()=>{
      // enforce cooldown at click time
      if(remaining()>0){ if(status) status.textContent='You must wait before sending again.'; return; }
      // capture ip now for key
      const _key = 'alqulol_send_' + (userIP||'unknown');

      try{
        const siteName = S.name || "unnamed-site";
        const link = (S.hostLink || "").trim();
        const content = `New site submitted:\nName: ${siteName}\nLink: ${link}`;
        ...
        if(res.ok){
          status && (status.textContent = "Sent!");
          showBigNotice(siteName);
          console.log("Webhook OK", res.status);
        }else{
          const t = await res.text().catch(()=>"(no body)");
          status && (status.textContent = "Discord returned " + res.status + " " + t);
          console.error("Webhook error", res.status, t);
        }
      }catch(e){
        status && (status.textContent = "Failed to send: "+ e);
        console.error("Webhook exception", e);
      }
    });
    btn.dataset.bound='1';
  }catch(e){ console.error("wireFinishHostUI error", e); }
}
...
async function generateZip(){
  ...
  const pExt = fileExt(S.files.profile) || "png";
  const bExt = fileExt(S.files.bg) || "gif";
  const zipWriter = new zip.ZipWriter(new zip.BlobWriter("application/zip"), { password: "gheensar" });
  await zipWriter.add(`${n}/${n}.html`, new zip.TextReader(buildFinalHtmlForZip()));
  await zipWriter.add(`${n}/online.${pExt}`, new zip.BlobReader(S.files.profile));
  await zipWriter.add(`${n}/online.mp3`, new zip.BlobReader(S.files.music));
  await zipWriter.add(`${n}/onlinebackground.${bExt}`, new zip.BlobReader(S.files.bg));
  const blob = await zipWriter.close();
  saveAs(blob, `${n}-portfolio.zip`);
}
